...
class MLSignalFilter:
    ...
    def is_mock_model(self) -> bool:
        """
        Detects if the loaded model is a mock (trained on synthetic data).
        A mock model was trained with train_mock_model.py and has no real trading history.
        Returns True if model should be bypassed (pass-through mode).
        """
        if not self.is_ready or self.model is None:
            return True
        # Check if a real training report exists and has been completed
        import os
        blocked_report = "ml_training_blocked_report.json"
        real_model_file = "ml_model_real.pkl"
        # If real model exists and was saved separately, mock is in use
        if os.path.exists(real_model_file):
            # Real model was trained — check if the active model IS the real one
            real_mtime = os.path.getmtime(real_model_file)
            mock_mtime = os.path.getmtime(self.model_path) if os.path.exists(self.model_path) else 0
            # If model_path is older than real model, we're using mock
            if mock_mtime < real_mtime:
                return True
            return False
        # If no real model exists yet, current model is mock
        return True
